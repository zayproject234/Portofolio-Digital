...
export default function Dashboard() {
  ...
  <img src={meImage} alt="Logo" width="100%" height="100%" style={{ objectFit: 'cover', borderRadius: '6px' }} />
</div>

                        <div>
                            <div className="brand-name">Yuzar Faisal</div>
                            <div className="brand-tag">Software Engineer</div>
                        </div>
                    </div>

                    <div className="stat-grid">
                        <div className="stat-card"><div className="stat-num">{totals.ongoing}</div><div className="stat-lbl">OngGoing</div></div>
                        <div className="stat-card"><div className="stat-num">{totals.paused}</div><div className="stat-lbl">Paused</div></div>
                        <div className="stat-card"><div className="stat-num">{totals.done}</div><div className="stat-lbl">Done</div></div>
                    ...
                    <nav>
                        {['all','ongoing','paused','done','transactions'].map(f => (
                            <button key={f} className={`nav-btn ${activeFilter===f? 'active':''}`} onClick={() => { setActiveFilter(f); }}>
                                {f==='all' ? 'All Project' : (f==='transactions' ? 'Transactions' : f.charAt(0).toUpperCase()+f.slice(1))}
                            </button>
                        ))}
                    </nav>

                    <button className="btn-add" onClick={() => openModal()}>+ Add Project</button>
                </aside>
